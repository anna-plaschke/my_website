...
description: Is the Weather Changing?
draft: false
image: pic08.jpg
keywords: ""
slug: blog5
...
**(1) - I select the year and the twelve month variables from the `weather` dataset. I will not use the others (J-D, D-N, DJF, etc.) for this analysis**

**(2) - I convert the dataframe from wide to 'long' format, that means that I create a separate row for every month. I name the new dataframe as `tidyweather`, the variable containing the name of the month as `month`, and the temperature deviation values as `delta`.**
...
**(3) - I will plot the data using a time-series scatter plot, and add a trendline. To do that, we first need to create a new variable called `date` in order to ensure that the `delta` values are plotted chronologically.**
...
```
In the scatterplot we see the monthly temperature deviations from 1880-2016 using as a base period the average between 1951-1980. The overall trend seems to be an increase in temperature deviations over the analysed time period. Since 1980, the temperature increase has been even higher. 


**(4) - Is the effect of increasing temperature more pronounced in some months? I use `facet_wrap()` to produce a separate scatter plot for each month, again with a smoothing line, to answer this question.**
...
```
**(5) - It is sometimes useful to group data into different time periods to study historical data. But first, I inspect the `comparison` dataframe by clicking on it in the `Environment` pane.**
...
**(6) - Now that I have the `interval` variable, I can create a density plot to study the distribution of monthly deviations (`delta`), grouped by the different time periods we are interested in. I set `fill` to `interval` to group and colour the data by different time periods.**
...
**(8) - I construct a confidence interval for the average annual delta since 2011, both using a formula and using a bootstrap simulation with the `infer` package.**
...
**(9) - So, what is the data showing us?**
