...
keywords: ""
slug: blog5
title: It is Getting Warm in Here!
...
I will plot the data using a time-series scatter plot, and add a trendline. To do that, we first need to create a new variable called `date` in order to ensure that the `delta` values are plotted chronologically.
...
**Density plot**
Now that I have the `interval` variable, I can create a density plot to study the distribution of monthly deviations (`delta`), grouped by the different time periods we are interested in. I set `fill` to `interval` to group and colour the data by different time periods.
...
**So, is it really getting war in here?**
